...
The Host Rules engine resolves named **capabilities** and **variants** from the current `(platform, version)`. See the [Host Rules guide](/guide/features/host-rules) for the narrative introduction; this page is the API reference.
...
A rule's `when` clause has optional `platform` and `version` keys; **at least one is required** (an empty `when: {}` throws at `defineHostRules()` time). A rule with only `platform` matches on platform alone; a rule with a `version` clause additionally requires the version constraint to pass (and is skipped when the version is unknown).
...
3. **Parse.** Unparsable version → `version: null`, but `versionRaw` keeps the original for diagnostics.
4. **Evaluate.** Capability: look up the resolved platform's rule (absent → `false`, boolean → itself, constraint → compare; unknown version → `false`). Variant: walk `rules` in order, first match wins, else `default`.
...
| `subscribe(listener)` | `() => void` | Subscribe to re-resolution (fires on `setVersion` / `refresh` / `__setOverride`); returns an unsubscribe. |
...
| `__setOverride(o)` | `void` | **Dev-only.** Force `{ platform?, version? }`, or `null` to clear. Not for production. |
...
Both built-ins are factories returning a `HostVersionSource` (`() => string | null`), so custom sources and built-ins share one type.
...
A **Host** tab appears showing the resolved platform, raw + parsed version, and every capability/variant with its resolved value, plus platform/version override controls (which call `host.__setOverride()`) and a reset button. The tab is omitted entirely when no `host` prop is passed, so the existing `DevToolsUI` API is unchanged.
